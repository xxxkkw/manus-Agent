...
class TemporalAttention(Layer):
    ...
    def call(self, x):
        ...
        exp = K.exp(S)
        S_normalized = exp / tf.reduce_sum(exp, axis=1, keepdims=True)
        return S_normalized
...
def diff_loss(diff, S):
    ...
    if len(S.shape) == 4:
        # batch input
        return tf.reduce_mean(tf.reduce_sum(tf.reduce_sum(diff ** 2, axis=3) * S, axis=(1, 2)))
    else:
        return tf.reduce_sum(tf.reduce_sum(diff ** 2, axis=2) * S)
...
def F_norm_loss(S, Falpha):
    ...
    if len(S.shape) == 4:
        # batch input
        return Falpha * tf.reduce_sum(tf.reduce_mean(S ** 2, axis=0))
    else:
        return Falpha * tf.reduce_sum(S ** 2)
...
class Graph_Learn(Layer):
    ...
    def call(self, x):
        ...
        for time_step in range(T):
            ...
            # normalization
            S = tmpS / tf.transpose(tf.broadcast_to(tf.reduce_sum(tmpS, axis=1), [V, N, V]), perm=[1, 2, 0])
...
class cheb_conv_with_Att_GL(Layer):
    ...
    def build(self, input_shape):
        ...
        #_, T, V, F = x_shape
        self.T = x_shape[1]  # 从静态input_shape获取时间步维度
        self.V = x_shape[2]  # 从静态input_shape获取顶点数
        self.F = x_shape[3]
        self.Theta = self.add_weight(name='Theta',
                                     shape=(self.k, self.F, self.num_of_filters),
                                     initializer='uniform',
                                     trainable=True)
...
def LayerNorm(x):
    ...
    layer_norm = tf.keras.layers.LayerNormalization(axis=3)
    ln = layer_norm(relu_x)
...
def GLGCN_Block(x, k, num_of_chev_filters, num_of_time_filters, time_conv_strides, time_conv_kernel, GLalpha, i=0):
    ...
    x_TAtt = Lambda(reshape_dot, 
                   name='reshape_dot' + str(i),
                   output_shape=lambda x: (x[0][0], x[0][1], x[0][2], x[0][3]))([x, temporal_Att])
    # spatial attention
    ...
    # end_output_GL = Lambda(LayerNorm, 
    #                       name='layer_norm' + str(2 * i),
    #                       output_shape=lambda x: x)(time_conv_output_GL)
    end_output_GL = tf.keras.layers.ReLU()(time_conv_output_GL)  
    end_output_GL = tf.keras.layers.LayerNormalization(
        axis=-1,  
        name='layer_norm' + str(2 * i)
    )(end_output_GL)

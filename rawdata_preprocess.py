...
output_filename = 'ISRUC_S3_single.npz' if single_mode else 'ISRUC_S3.npz'
np.savez(path.join(path_output, output_filename),
        Fold_data=np.array(fold_psg, dtype=object),
        Fold_label=np.array(fold_label, dtype=object),
        Fold_len=np.array(fold_len),
        allow_pickle=True
        )
print('Saved to', path.join(path_output, output_filename))
